...
LOG_FILE = "Suporte_SW.log"  # Nome fixo em ASCII para funcionar na VPS
logging.basicConfig(
    level=logging.INFO,
...
class WebScraper:
    ...
    def __init__(self):
        ...
        self.token = "8776560692:AAGz4SNe2VblKYUS70zZHp2CyqTeeCX95mg" # config
    ...
    def _send_with_retry(self, func, *args, max_retries=2, **kwargs):
        """Tenta enviar mensagem ao Telegram com retry"""
        for attempt in range(max_retries):
            try:
                return func(*args, **kwargs)
            except Exception as e:
                log.warning("Falha ao enviar para Telegram: %s", e)
                if attempt < max_retries - 1:
                    time.sleep(1)
                    continue
                else:
                    if self.telegram_connected:
                        log.warning("Sem conexão com Telegram - continuando análise...")
...
📢 Proteção no ⚪️""",
    ...
    def start(self):
        ...
        while True:
            try:
                ...
                response = requests.get(self.url_API, timeout=REQUESTS_TIMEOUT)
                response.raise_for_status()
                json_data = json.loads(response.text)
                if not isinstance(json_data, list):
                    log.warning("API não retornou lista: %s", type(json_data))
                    continue
                for i in json_data:
                ...
                log.warning("Timeout na API - tentando de novo...")
                time.sleep(5)
                continue
            except requests.exceptions.RequestException as e:
                log.warning("Erro de rede/API: %s - tentando de novo...", e)
                time.sleep(5)
                continue
            except (json.JSONDecodeError, KeyError) as e:
                log.warning("Resposta da API inválida: %s - tentando de novo...", e)
                time.sleep(5)
                continue
            except Exception as e:
    ...
    def estrategy(self, results):
        ...
        # ESTRATÉGIAS COM BASE NO CSV - LÓGICA CORRIGIDA
            ...
            try:
                f = open("_blaze_estrategyG1VIP-WS.csv", newline="", encoding="utf-8")
            except FileNotFoundError:
                log.warning("Arquivo _blaze_estrategyG1VIP-Cpro.csv não encontrado - verifique o caminho na VPS.")
                return
            except OSError as e:
                log.warning("Erro ao abrir CSV: %s", e)
                return
            with f:
                reader = csv.reader(f)
...
def main():
    while True:
        try:
            log.info("Iniciando script %s...", os.path.basename(__file__))
            scraper = WebScraper()
            scraper.start()
            ...
            log.error("Script parou inesperadamente: %s\n%s", e, traceback.format_exc())
            log.info("Reiniciando em 30 segundos...")
            time.sleep(30)
